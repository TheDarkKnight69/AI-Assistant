...
def ai(query):
    ...
    elif any(query.startswith(keyword) for keyword in question):
        if "weather" in query:
            return weather()
    ...
    elif ("take a note" in query or "note down" in query or "add a note" in query or 'create note' in query):
        try:
            with open('notes.json', 'r') as f:
                data = json.load(f)
        except FileNotFoundError:
    ...
    elif ("show me my notes" in query or "list notes" in query):
        try:
            with open('notes.json', 'r') as f:
                data = json.load(f)
            a = "\nNotes: \n"
            ...
            for ke in keys:
                i +=1
            a += "Searching for Notes....\n"
            speak("Searching for Notes....")
            print()
            a+= f"You have {num2words(i)} Notes.\n"
            speak(f"You have {num2words(i)} Notes.")
            a +=("Reading Note Headlines now:\n\n")
